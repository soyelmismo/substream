// Package ctrlsubsonic provides a simplified cache system for Substream.
//
// This package contains a type-safe LRU + TTL cache system using generics.
// Designed for single instances with few users.
//
// BASIC USAGE:
//
//	// Create cache
//	cache := NewCache[int](CacheConfig{
//	    Name:            "genre-tracks",
//	    MaxSize:         100,
...
//
//	BEFORE:
//	  c.genreCache.set(key, trackIDs, genreCacheTTL)
//	  cached := c.genreCache.get(key)
//
//	AFTER:
//	  c.genreCache.Set(key, trackIDs, 0)
//	  cached := c.genreCache.Get(key)
//
// RECOMMENDED CONFIGURATION FOR FEW USERS:
//
//	TidalProxy:  MaxSize=200, TTL=2h
//	GenreTracks: MaxSize=50,  TTL=1h
//	Search:      MaxSize=100, TTL=30m
//
// FEATURES:
//
...
//
//	Time Complexity:
//	- Get:        O(1) - map lookup + LRU list update
//	- Peek:       O(1) - map lookup only (no LRU update)
//	- Set:        O(1) - map insert + LRU list push
//	- Delete:     O(1) - map delete + LRU list remove
//	- GetOrSet:   O(1) - same as Get or Set
//	- GetWithLoader: O(1) for cache hit, O(loader) for cache miss
//	- Clear:      O(1) - map reinitialization + list reset
//	- Size:       O(1) - list length
//	- Stats:      O(1) - atomic loads
//	- cleanupExpired: O(n) - iterates all entries (background)
//
//	Space Complexity:
//	- O(n) where n is the number of cached entries
//	- Each entry stores: key (string), value (T), expiry (time.Time), LRU pointer
//
//	Thread Safety:
//	- All operations are thread-safe using sync.RWMutex
//	- Read operations (Peek, Size, Stats) use RLock for concurrent reads
//	- Write operations (Set, Get, Delete, etc.) use Lock for exclusive access
//	- Stats counters use atomic.Int64 for lock-free reads
//	- Cleanup goroutine runs independently without blocking cache operations
